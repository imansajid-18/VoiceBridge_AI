...
function Register() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL ?? 'http://127.0.0.1:8000/api'}/register/`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-stage flex items-center justify-center p-6 text-white relative overflow-hidden">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h1 className="text-2xl font-extrabold text-white mb-1 tracking-tight text-center">Create account</h1>
          <p className="text-sm text-white/50 mb-7 text-center font-medium">Sign up for VoiceBridge</p>
          ...
          <Link to="/login" className="w-full text-center bg-white/10 border border-white/20 hover:bg-white/15 text-white rounded-full py-3.5 text-sm font-semibold transition-all active:scale-[0.98]">
            Log in instead
          </Link>
